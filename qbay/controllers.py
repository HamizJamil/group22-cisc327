...
def create_product():
    if request.method == "POST":
        ...
        if title.startswith(" ") or title.endswith(" ") or not title.replace(" ","").isalnum():
        ...
        if found_Prod:
            flash("This product is already created! Please use update or add a new title")
            return render_template("createproduct.html")
        else:
...
def registration():
    if request.method == "POST":
        ...
        if email_regex.search(email) is None: # if email provided is not RFC 5322
